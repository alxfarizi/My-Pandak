...
class AuthService {
  ...
  // FIXED: Update method updateProfile dengan sync ke anggota_keluarga

  static Future<bool> updateProfile(Map<String, dynamic> updates) async {
    if (!isLoggedIn) return false;

    try {
      // FIXED: Handle NIK update with sync
      if (updates.containsKey('nik')) {
        ...
        // FIXED: Check availability with correct function name
        final result = await _client.rpc(
          'check_nik_availability_for_users', // FIXED: Correct function name
          params: {
        ...
        // FIXED: Update NIK with sync to anggota_keluarga
        final syncResult = await _client.rpc(
          'update_user_nik_with_sync',
          params: {
            'user_auth_id': currentUser!.id,
            'new_nik': newNik.trim(),
          },
        ...
        if (syncResult != true) {
          throw Exception('Gagal sync NIK ke data warga');
        }

        // Remove NIK from updates since it's already handled by the function
        updates = Map<String, dynamic>.from(updates);
        updates.remove('nik');
      }
