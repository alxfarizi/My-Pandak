...
class PreviewDataKeluargaPage extends StatelessWidget {
  ...
  Widget _paperTitle(String text) {
    return Center(child: Text(text, style: const TextStyle(fontSize: 7, fontWeight: FontWeight.w600, color: Colors.black, fontFamily: _font)));
  }
  ...
  Widget _buildPaper1Content() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _buildFormRowDotted('Desa Wisma', keluargaData['desaWisma'] ?? keluargaData['mawar'] ?? ''),
          SizedBox(height: _vspaceXS),
          _buildFormRowDotted('Nama Kepala Rumah Tangga', keluargaData['nama'] ?? ''),
          SizedBox(height: _vspaceXS),
          _buildFormRowDotted('Dusun / Lingk', _composeSlash(keluargaData['dusun'], keluargaData['lingkungan'])),
          SizedBox(height: _vspaceXS),
          _buildFormRowDotted('RT / RW', _composeSlash(keluargaData['rt'], keluargaData['rw'])),
          ...
          Row(children: [
            ...
            Expanded(child: Wrap(spacing: 8, runSpacing: 6, crossAxisAlignment: WrapCrossAlignment.center, children: [
              _inlineCountPreview(keluargaData['jumlahAnggota'], 'Orang'),
              _inlineLabeledCountPreview('Laki-laki', keluargaData['jumlahLaki'], 'Orang'),
              _inlineLabeledCountPreview('Perempuan', keluargaData['jumlahPerempuan'], 'Orang'),
            ])),
          ]),
          const SizedBox(height: 6),
          _buildFormRowDotted('1. Jumlah KK', keluargaData['jumlahKk'] ?? ''),
          const SizedBox(height: 4),
          _buildJumlahSection(),
          const SizedBox(height: 6),
          _buildLandscapeTableHeader(),
          const SizedBox(height: 6),
          _buildLandscapeTableRow('330222110210001', 'Susilo Indra.P', 'Suami', 'Kawin', '01/01/1980', 'SMU', 'Buruh', male: true),
          _buildLandscapeTableRow('3302225010840003', 'Reny Budi Utami', 'Istri', 'Kawin', '10/10/1984', 'SMK', 'Mengurus RT', female: true),
          _buildLandscapeTableRow('3302224206060001', 'Meisya Ratu.C', 'Anak', 'Belum Kawin', '06/06/2006', 'SMU', 'Pelajar', female: true),
        ]),
      ),
    ]);
  }
  ...
  Widget _buildPaper2Content() {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _buildCheckboxGroupPreview('3. Makanan Pokok Sehari-hari', ['Beras', 'Non Beras'], keluargaData['makananPokok']),
        const SizedBox(height: 8),
        _buildJambanPreview(),
        const SizedBox(height: 8),
        _buildCheckboxGroupPreview('5. Sumber Air Keluarga', ['PDAM', 'Sumur', 'Lainnya'], keluargaData['sumberAir']),
        const SizedBox(height: 8),
        _buildCheckboxGroupPreview('6. Memiliki Tempat Pembuangan Sampah', ['Ya', 'Tidak'], keluargaData['tempatSampah']),
        const SizedBox(height: 8),
        _buildCheckboxGroupPreview('7. Mempunyai Saluran Pembuangan Air Limbah', ['Ya', 'Tidak'], keluargaData['saluranAirLimbah']),
        const SizedBox(height: 8),
        _buildCheckboxGroupPreview('8. Menempel Stiker P4K', ['Ya', 'Tidak'], keluargaData['menempelStikerP4k']),
        const SizedBox(height: 8),
        _buildCheckboxGroupPreview('9. Kriteria Rumah', ['Ya', 'Tidak'], keluargaData['kriteriaRumah']),
        const SizedBox(height: 8),
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _buildCheckboxGroupPreview('10. Aktivitas UP2K', ['Ya', 'Tidak'], keluargaData['aktivitasUp2k']),
          const SizedBox(height: 6),
          if ((keluargaData['aktivitasUp2k'] ?? '') == 'Ya') Row(children: [
            _checkboxLabel('Warung', (keluargaData['jenisUsahaPilihan'] ?? '') == 'Warung'),
            const SizedBox(width: 12),
            _checkboxLabel('Kegiatan Koperasi', (keluargaData['jenisUsahaPilihan'] ?? '') == 'Kegiatan Koperasi'),
          ]),
        ]),
        const SizedBox(height: 8),
        _buildCheckboxGroupPreview('11. Aktivitas Kegiatan Usaha Kesehatan Lingkungan', ['Layak', 'Tidak Layak'], keluargaData['aktivitasKegiatanKesehatan']),
      ]),
    );
  }
  ...
  Widget _buildChecklistBlock(List<String> items) {
    return Wrap(runSpacing: 4, spacing: 12, children: items.map((i) => _buildCheckbox(i)).toList());
  }
  ...
  Widget _buildNumber(String txt) {
    return SizedBox(width: _numberWidth, child: Text(txt, style: const TextStyle(fontSize: 5, color: Colors.black, fontFamily: _font)));
  }
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        Expanded(child: Wrap(spacing: 12, runSpacing: 4, children: [
          _checkboxLabel('Ya', (keluargaData['jambanKeluarga'] ?? '') == 'Ya'),
          _checkboxLabel('Tidak', (keluargaData['jambanKeluarga'] ?? '') == 'Tidak'),
        ])),
      ]),
      const SizedBox(height: 4),
      Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        const Text(':', style: TextStyle(fontSize: 5, fontFamily: _font)),
        const SizedBox(width: 3),
        SizedBox(width: 25, child: (keluargaData['jumlahJambanOrang'] ?? '').isNotEmpty ? Center(child: Text(keluargaData['jumlahJambanOrang']!, style: const TextStyle(fontSize: 5, fontFamily: _font))) : _dottedUnderline()),
        const SizedBox(width: 4),
    ...
    return Row(children: [
      ...
      Expanded(child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _inlineLabeledCountPreview('A). Balita', keluargaData['balita'], 'Anak'),
          const SizedBox(height: 6),
          _inlineLabeledCountPreview('C). WUS', keluargaData['wus'], 'Orang'),
          const SizedBox(height: 6),
          _inlineLabeledCountPreview('E). Ibu Hamil', keluargaData['ibuHamil'], 'Orang'),
          const SizedBox(height: 6),
          _inlineLabeledCountPreview('G). Lansia', keluargaData['lansia'], 'Orang'),
        ])),
        const SizedBox(width: 8),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _inlineLabeledCountPreview('B). Plus', keluargaData['pus'], 'Pasang'),
          const SizedBox(height: 6),
          _inlineLabeledCountPreview('D). Buta', keluargaData['buta'], 'Orang'),
          const SizedBox(height: 6),
          _inlineLabeledCountPreview('F). Ibu Menyusui', keluargaData['ibuMenyusui'], 'Orang'),
          const SizedBox(height: 6),
          Row(children: [
            _checkboxLabel('Fisik', keluargaData['lansiaKriteria'] == 'Fisik'),
            const SizedBox(width: 12),
            _checkboxLabel('Non Fisik', keluargaData['lansiaKriteria'] == 'Non Fisik'),
          ]),
        ])),
      ])),
    ]);
  ...
  Widget _buildCheckbox(String label) {
    return Row(mainAxisSize: MainAxisSize.min, children: [
      Container(width: 8, height: 8, decoration: BoxDecoration(border: Border.all(color: Colors.black, width: 1), borderRadius: BorderRadius.circular(2), color: Colors.white)),
      const SizedBox(width: 4),
