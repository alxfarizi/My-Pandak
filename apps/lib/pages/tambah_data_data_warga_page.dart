...
class _TambahDataWargaPageState extends State<TambahDataWargaPage> {
  ...
  void initState() {
    ...
    if (init != null) {
      _desaWismaController.text = init.mawar;
      _namaController.text = init.nama;
      _noKtpNikController.text = init.nik;
      _namaKepalaController.text = init.namaKepala ?? '';
      _noRegistrasiController.text = init.noRegistrasi ?? '';
      _jabatanController.text = init.jabatan ?? '';
      _jenisKelamin = init.jenisKelamin;
      _tempatLahirController.text = init.tempatLahir ?? '';
      _tanggalLahirController.text = init.tanggalLahir ?? '';
      _bulanLahirController.text = init.bulanLahir ?? '';
      _tahunLahirController.text = init.tahunLahir ?? '';
      _umurController.text = init.umur ?? '';
      _desaKelController.text = init.desaKel ?? '';
      _kabKotaController.text = init.kabKota ?? '';
      _alamatController.text = init.alamat ?? '';
      _statusPerkawinan = init.statusPerkawinan;
      _statusDalamKeluarga = init.statusDalamKeluarga;
      _agama = init.agama;
      _statusTinggal = init.statusTinggal;
      _pendidikan = init.pendidikan;
      _pekerjaan = init.pekerjaan;
      _akseptorKb = init.akseptorKb;
      _aktifPosyandu = init.aktifPosyandu;
      _frekuensiPosyanduController.text = init.frekuensiPosyandu ?? '';
      _binaBalita = init.binaBalita;
      _memilikiTabungan = init.memilikiTabungan;
      _paketTabungan = init.paketTabungan;
      _ikutPaud = init.ikutPaud;
      _ikutKoperasi = init.ikutKoperasi;
      _berkebutuhanKhusus = init.berkebutuhanKhusus;
      _jenisAkseptorKbController.text = init.jenisAkseptorKb ?? '';
    }
  }
        ...
        child: Column(
          ...
          children: [
            ...
            if (_aktifPosyandu == 'Ya')
              Padding(padding: _sectionPadding, child: _buildFrequencyInput()),
            const SizedBox(height: _gapMedium),
            _buildCheckboxGroup(
              ...
              children: [
                if (widget.initial != null)
                  OutlinedButton.icon(
                    onPressed: () async {
                      ...
                      if (id != null) {
                        await _controller.deleteWarga(id);
                      }
                ...
                ElevatedButton.icon(
                  onPressed: () async {
                    ...
                    final nik = _noKtpNikController.text.trim();
                    if (nama.isNotEmpty && mawar.isNotEmpty && nik.isNotEmpty) {
                      final data = Warga(
                        ...
                        jenisAkseptorKb: _jenisAkseptorKbController.text.trim(),
                        aktifPosyandu: _aktifPosyandu,
                        frekuensiPosyandu: _frekuensiPosyanduController.text
                            .trim(),
                        binaBalita: _binaBalita,
                        memilikiTabungan: _memilikiTabungan,
                        paketTabungan: _paketTabungan,
                        ikutPaud: _ikutPaud,
                        ikutKoperasi: _ikutKoperasi,
                        berkebutuhanKhusus: _berkebutuhanKhusus,
                      );
                      if (widget.initial?.id != null) {
                        await _controller.updateWarga(data);
                      } else {
                        await _controller.addWarga(data);
                      }
                    }
                    Navigator.pop(context, true);
                  },
