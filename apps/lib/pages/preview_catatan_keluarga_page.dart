...
class PreviewCatatanKeluargaPage extends StatelessWidget {
        ...
        child: Column(children: [
          Padding(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('Ukuran : A4', style: TextStyle(fontSize: 12, color: Colors.black, fontFamily: _font)),
              Container(
                width: 45,
                height: 45,
                ...
                child: Stack(alignment: Alignment.center, children: const [
                  ...
                  Positioned(bottom: 8, child: Text('Download', style: TextStyle(fontSize: 5, fontWeight: FontWeight.w700, color: Colors.white, fontFamily: _font))),
                ]),
              ),
            ]),
          ),
          Expanded(
            child: SingleChildScrollView(
              child: Column(children: [
                _buildLandscapePaper(context, pageIndex: 1, child: _buildPageContent()),
              ]),
            ),
          ),
        ]),
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('$pageIndex/1', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w500, color: Colors.white, fontFamily: _font)),
  ...
  Widget _buildPageContent() {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Center(child: Text('Catatan Keluarga', style: const TextStyle(fontSize: 7, fontWeight: FontWeight.w600, fontFamily: _font))),
        SizedBox(height: _vspaceS),
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _formRowDotted('Catatan Keluarga dari', catatanData['catatanDari'] ?? catatanData['nama'] ?? ''),
            SizedBox(height: _vspaceXS),
            _formRowDotted('Anggota Kelompok Dasa Wisma', catatanData['anggotaKelompok'] ?? catatanData['mawar'] ?? ''),
            SizedBox(height: _vspaceXS),
            _formRowDotted('Tahun', catatanData['tahun'] ?? ''),
          ])),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _checkboxGroupPreview('Kriteria Rumah', ['Sehat', 'Tidak Sehat'], catatanData['kriteriaRumah']),
            SizedBox(height: _vspaceXS),
            _checkboxGroupPreview('Anggota Kelompok Dasa Wisma', ['Ada', 'Tidak'], catatanData['anggotaKelompokAda']),
            SizedBox(height: _vspaceXS),
            _checkboxGroupPreview('Tempat Sampah', ['Ada', 'Tidak'], catatanData['tempatSampah']),
          ])),
        ]),
        const SizedBox(height: 10),
        _catatanTableHeader(),
        _catatanTableRow(_sampleRow(1, 'Susilo I.P', 'Menikah', 'L', 'Banyumas', '21/10/1981', 'Islam', 'SMU', 'Buruh')),
        _catatanTableRow(_sampleRow(2, 'Reny B.L', 'Menikah', 'P', 'Banyumas', '10/10/1984', 'Islam', 'SMK', 'IRT')),
        _catatanTableRow(_sampleRow(3, 'Neisya.R.', 'Lajang', 'P', 'Banyumas', '02/06/2006', 'Islam', 'SMU', 'Pelajar')),
        _catatanTableRow(_sampleRow(4, 'Dafeena', 'Lajang', 'P', 'Banyumas', '01/12/2021', 'Islam', 'Belum', 'Belum')),
      ]),
    );
  }

  Map<String, String> _sampleRow(int no, String nama, String status, String lp, String tempat, String tgl, String agama, String pendidikan, String pekerjaan) {
    return {
      'no': '$no',
      'nama': nama,
  ...
  Widget _catatanTableHeader() {
    ...
    final flex = const [6, 18, 12, 6, 14, 18, 10, 14, 14, 12, 18, 14, 18, 18, 14, 12, 12, 14, 10];
    return Column(children: [
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Row(children: [
            for (int i = 0; i < 9; i++) Expanded(flex: flex[i], child: Container(decoration: BoxDecoration(border: Border.all(color: Colors.black, width: 1)))),
            Expanded(
              flex: flex.sublist(9, 18).reduce((a, b) => a + b),
              child: Container(
                decoration: BoxDecoration(border: Border.all(color: Colors.black, width: 1)),
                child: const Center(child: Text('Pekerjaan', style: TextStyle(fontSize: 6, fontWeight: FontWeight.w600, fontFamily: _font))),
              ),
            ),
            Expanded(flex: flex[18], child: Container(decoration: BoxDecoration(border: Border.all(color: Colors.black, width: 1)))),
          ]),
        ),
        Row(children: [
          for (int i = 0; i < labels.length; i++)
            Expanded(
              ...
              child: Container(
                height: 32,
                decoration: BoxDecoration(color: const Color(0xFFF5F5F5), border: Border.all(color: Colors.black, width: 1)),
                child: Center(child: Text(labels[i], textAlign: TextAlign.center, style: const TextStyle(fontSize: 5, fontWeight: FontWeight.w500, color: Colors.black, fontFamily: _font))),
              ),
            ),
        ]),
        Row(children: [
          for (int i = 0; i < flex.length; i++)
            Expanded(
              ...
              child: Container(
                height: 16,
                decoration: BoxDecoration(border: Border.all(color: Colors.black, width: 1)),
                child: Center(child: Text((i + 1).toString(), style: const TextStyle(fontSize: 5, fontFamily: _font))),
              ),
            ),
        ]),
      ]),
        ...
        for (int i = 0; i < flex.length; i++)
          Expanded(
            ...
            child: Container(
              ...
              child: Center(child: Text(cells[i], style: const TextStyle(fontSize: 5, fontFamily: _font))),
            ),
          ),
      ]),
    );
  ...
  Widget _formRowDotted(String label, String value) {
    return Row(children: [
      ...
      Expanded(child: value.isNotEmpty ? Text(value, style: const TextStyle(fontSize: 5, fontFamily: _font)) : _dottedUnderline()),
    ]);
  }
  ...
  Widget _checkboxGroupPreview(String label, List<String> options, String? selected) {
    return Row(children: [
      SizedBox(width: _labelWidth, child: Text(label, style: const TextStyle(fontSize: 5, fontFamily: _font))),
      SizedBox(width: _colonWidth, child: const Text(':', style: TextStyle(fontSize: 5, fontFamily: _font))),
      const SizedBox(width: _afterColonSpacing),
  ...
  Widget _jambanPreview() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        SizedBox(width: _labelWidth, child: const Text('Jamban Keluarga', style: TextStyle(fontSize: 5, fontFamily: _font))),
        SizedBox(width: _colonWidth, child: const Text(':', style: TextStyle(fontSize: 5, fontFamily: _font))),
        const SizedBox(width: _afterColonSpacing),
        Expanded(child: Wrap(spacing: 12, runSpacing: 4, children: [
          _checkboxLabel('Ya', (catatanData['jambanKeluarga'] ?? '') == 'Ya'),
          _checkboxLabel('Tidak', (catatanData['jambanKeluarga'] ?? '') == 'Tidak'),
        ])),
      ]),
      ...
      Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(width: 3),
        SizedBox(width: 45, child: (catatanData['jumlahJambanOrang'] ?? '').isNotEmpty ? Center(child: Text(catatanData['jumlahJambanOrang']!, style: const TextStyle(fontSize: 5, fontFamily: _font))) : _dottedUnderline()),
        const SizedBox(width: 4),
