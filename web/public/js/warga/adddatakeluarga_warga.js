...
function goBack() {
    window.location.href = 'datakeluarga_warga';
}
...
function loadTableData(tableIndex, data) {
    ...
    // Load text inputs
    const textFields = ['noRegistrasi', 'namaAnggota', 'tglLahir', 'umur', 'pekerjaan', 'lainnyaStatus'];
    textFields.forEach(field => {
        ...
        if (element && data[field]) {
            ...
            if (field === 'tglLahir' && data[field]) {
                ...
                // Trigger calculation logic if needed, but since we saved 'umur', we can just load it
            }
        }
    });
...
function saveData() {
    ...
    const existingData = JSON.parse(localStorage.getItem('dataKeluargaWarga') || '[]');
    existingData.push(formData);
    localStorage.setItem('dataKeluargaWarga', JSON.stringify(existingData));
