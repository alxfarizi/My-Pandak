// Data warga (diambil dari localStorage khusus Warga)
let dataWarga = JSON.parse(localStorage.getItem('dataWargaWarga') || '[]');
    ...
    dataWarga.forEach((warga, index) => {
        ...
        row.addEventListener('click', (e) => {
            ...
            if (!e.target.closest('.action-buttons')) {
                window.location.href = `adddatawarga_warga?id=${warga.id}`;
            }
        });
...
function deleteData(id) {
    if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
        ...
        localStorage.setItem('dataWargaWarga', JSON.stringify(dataWarga));
        renderTable();
