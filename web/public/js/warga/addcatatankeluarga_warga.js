...
function goBack() {
    window.location.href = 'catatankeluarga_warga';
}
...
function setupCalendarIcon(tableIndex) {
    ...
    const umurInput = document.getElementById(`umur_${tableIndex}`);
    ...
    newTglLahirInput.addEventListener('input', function() {
        ...
        if (this.value === '') {
            ...
            if (umurInput) umurInput.value = '0';
        }
    });
...
function saveCurrentTableData() {
    const tableData = {
        ...
        umur: document.getElementById(`umur_${currentTableIndex}`)?.value || '0',
        pekerjaan: document.getElementById(`pekerjaan_${currentTableIndex}`)?.value || ''
    };
    
    // Save checkbox values
    const checkboxGroups = [
        'statusPerkawinan', 'jenisKelamin', 'agama', 'pendidikan', 'berkebutuhanKhusus'
    ];
...
function loadTableData(tableIndex, data) {
    ...
    // Load text inputs
    const textFields = ['namaAnggota', 'tglLahir', 'umur', 'pekerjaan'];
    ...
    // Load checkboxes
    const checkboxGroups = ['statusPerkawinan', 'jenisKelamin', 'agama', 'pendidikan', 'berkebutuhanKhusus'];
    checkboxGroups.forEach(groupName => {
        if (data[groupName]) {
...
function clearTableForm() {
    // Clear text inputs
    const inputs = document.querySelectorAll('.table-form input[type="text"]');
    inputs.forEach(input => {
        input.value = '';
        input.classList.remove('has-value');
        if (input.id.startsWith('umur')) input.value = '0'; // Reset umur to 0
    });
...
function setupTableCheckboxes() {
    const checkboxGroups = [
        `statusPerkawinan_${currentTableIndex}`,
        `jenisKelamin_${currentTableIndex}`,
        `agama_${currentTableIndex}`,
        `pendidikan_${currentTableIndex}`,
        `berkebutuhanKhusus_${currentTableIndex}`
    ];
...
function collectAllFormData() {
    ...
    const formData = {
        ...
        // Nama KRT
        namaKRT: document.getElementById('namaKRT').value,
        namaKRT2: document.getElementById('namaKRT2').value,
        
        // Peternakan
        peternakanKomoditi: document.getElementById('peternakanKomoditi').value,
        peternakanVolume: document.getElementById('peternakanVolume').value,
        
        // Pertanian
        pertanianKomoditi: document.getElementById('pertanianKomoditi').value,
        pertanianVolume: document.getElementById('pertanianVolume').value,
        
        // Toga
        togaKomoditi: document.getElementById('togaKomoditi').value,
        togaVolume: document.getElementById('togaVolume').value,
        
        // Lumbung Hidup 1
        lumbungKomoditi: document.getElementById('lumbungKomoditi').value,
        lumbungVolume: document.getElementById('lumbungVolume').value,
        
        // Lumbung Hidup 2
        lumbungKomoditi2: document.getElementById('lumbungKomoditi2').value,
        lumbungVolume2: document.getElementById('lumbungVolume2').value,
        
        // Tanaman Keras
        tanamanKerasKomoditi: document.getElementById('tanamanKerasKomoditi').value,
        tanamanKerasVolume: document.getElementById('tanamanKerasVolume').value,
        
        // Industri Rumah Tangga
        panganKomoditi: document.getElementById('panganKomoditi').value,
        panganVolume: document.getElementById('panganVolume').value,
        sandangKomoditi: document.getElementById('sandangKomoditi').value,
        sandangVolume: document.getElementById('sandangVolume').value,
        jasaKomoditi: document.getElementById('jasaKomoditi').value,
        jasaVolume: document.getElementById('jasaVolume').value,
        lainLainKomoditi: document.getElementById('lainLainKomoditi').value,
        lainLainVolume: document.getElementById('lainLainVolume').value
    };
    
    // Checkbox groups
    const checkboxGroups = [
        'kriteriaRumah', 'jambanKeluarga', 'tempatSampah',
        'pancasila', 'gotongRoyong', 'pendidikanKetrampilan', 'koperasi',
        'pangan', 'sandang', 'kesehatan', 'perencanaanSehat', 'kelestarian'
    ];
...
function validateForm() {
    const requiredFields = [
        { id: 'namaKeluarga', label: 'Catatan Dari Keluarga' },
        { id: 'kelompok', label: 'Anggota Dari Kelompok' },
...
function saveData() {
    ...
    const existingData = JSON.parse(localStorage.getItem('dataCatatanWarga') || '[]');
    existingData.push(formData);
    localStorage.setItem('dataCatatanWarga', JSON.stringify(existingData));
    
    // Tampilkan notifikasi sukses
    alert('Data berhasil disimpan!');
    
    // Redirect kembali ke halaman catatan keluarga
    window.location.href = 'catatankeluarga_warga';
}
...
function setupInputColorChange() {
    ...
    const numberInputs = document.querySelectorAll('.form-input[type="number"], .inline-number-input');
...
// Setup conditional untuk Jamban Keluarga
function setupJambanConditional() {
    const jambanAda = document.querySelector('input[name="jambanKeluarga"][value="Ada"]');
    const jambanTidak = document.querySelector('input[name="jambanKeluarga"][value="Tidak Ada"]');
    ...
    jambanAda.addEventListener('change', function() {
        if (this.checked) {
            enableJumlahJamban();
        }
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('namaKeluarga').focus();
});

// Prevent form submission on Enter key
document.getElementById('formCatatanKeluarga').addEventListener('submit', function(e) {
    e.preventDefault();
    return false;
});
